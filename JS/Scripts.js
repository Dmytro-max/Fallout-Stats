...
function Skill_up(name, char) {
  ...
  //if NOT our defined step smaller or equal to the points left, - we set step to the number points that left
  step =
    char.skillPoints_perUp <= skill_points - spent
      ? char.skillPoints_perUp
      : skill_points - spent;

  if (skill_points > spent) {
    step =
      skillvalue + step <= char.maxSkillValue
        ? step
        : char.maxSkillValue - skillvalue;
  }
...
function Skill_down(name, char) {
  ...
  char.skillPoints_perDown <= skill_Bonus
    ? (step = char.skillPoints_perDown)
    : (step = 1);
...
function Special_up(char, name) {
  if (
    ...
    char.SPECIAL[name].value < char.maxSpecialValue
  ) {
...
function Choose_trait(char, elem, name) {
  if (char.level == 1) {
    if (elem.classList.contains("unChecked") && char.traits_Aveilible >= 1) {
      //make sure value is not too big
...
function Ability_Add(char, name) {
  ...
  // try {
  //   condition =
  //     !char.Abilities_Availible.get(name).classList.contains("unAvailible");
  // } catch (error) {
  //   debugger;
  // }
  let perk = char.Main_Abilities[name];
  if (char.Abilitie_IsAvailible(perk, name)) {
    switch (perk.type) {
      case "levelup":
      case "special":
      case "implant":
    ...
    ChangeInfo();
    function ChangeInfo() {
      let added_block = CharacterForm.Abilities_Added.get(name);
      if (perk.RangsAdded?.()) {
      ...
      added_block.querySelector(
        "h4.rang"
      ).textContent = `${perk.rang}/${perk.rangs}`;
      added_block.querySelector("h4.level").textContent = `${perk.level_taken}${
        perk.type == "levelup" && perk?.level != undefined
          ? "|" + perk?.level
          : ""
      }`;
      added_block.style.display = "grid";

      let av_block = CharacterForm.Abilities_Availible.get(name);
      av_block.querySelector(
        "h4.rang"
      ).textContent = `${perk.rang}/${perk.rangs}`;
...
function Ability_Remove(char, name) {
  let perk = char.Main_Abilities[name];
  if (char.Abilitie_IsRemovable(perk, name)) {
    perk.Remove?.(char);
    perk.rang -= 1;
...
function LevelUp(char) {
  ...
  if (
    level_reached > char.level &&
    char.Special_BonusPoints == 0 &&
    char.prizeSkillsAveilible == 0
  ) {
    //Checking if there are taken level_reached for restoring
    RestoringLevel();
  } else if (
    level_reached == 1 &&
    char.Special_BonusPoints == 0 &&
  ...
  // and has to be chosen or Perk is chosen
    ...
    char.Special_BonusPoints == 0 &&
    (!char.IsPerkLevel() ||
      (char.IsPerkLevel() && char.PerksbyLevel.has(char.level)
        ? char.PerksbyLevel.get(char.level)["levelup"]
        : false))
  ) {
    NewLevel();
  }
  ...
  for (let key in char.skills) {
    char.skillBookBlocks.get(key)["select"].selectedIndex =
      CharacterlevelSelector.selectedIndex;
  }
  ...
  function ToSecondLevel() {
    CharacterlevelSelector.insertAdjacentHTML(
      "beforeend",
      `<option value="${char.level + 1}">${char.level + 1}</option>`
    );
    CharacterlevelSelector.selectedIndex += 1;
    ...
    char.skillsByLevel[char.level - 1]["points"] = Math.floor(
      char.SkillPointsCount()
    );
    // console.log("Skillpoints rest: " + char.rest_skillPoints  );
    // console.log("Skillpoints rest: " + char["rest_skillPoints  "]);
    char.rest_skillPoints +=
      char.SkillPointsCount() - Math.floor(char.SkillPointsCount());

    //CHECK LATER
    if (char.rest_skillPoints >= 1) {
      //!! Counted skillpoints number can be float, so Integer part must be used already and floating part should be accumulated through different levels
      char.skillsByLevel[char.level - 1]["points"] += Math.floor(
        char.rest_skillPoints
      );
      char.rest_skillPoints -= Math.floor(char.rest_skillPoints);
    }
  }

  //??
  function RestoringLevel() {
    console.log("Restoring level: " + level_reached);
    char.level += 1;
    if (char.PerksbyLevel.has(char.level)) {
    ...
    for (let key in char.skills) {
      ...
      if (
        ...
        Math.floor(char.SkillPointsCount())
      ) {
  ...
  function NewLevel() {
    ...
    char.skillsByLevel[char.level - 1]["skillbook_bonus"] =
      char.skillbook_bonus;

    char.skillsByLevel[char.level - 1]["points"] = Math.floor(
      char.SkillPointsCount() + Math.floor(char.rest_skillPoints)
    );
    char.rest_skillPoints +=
      char.SkillPointsCount() - Math.floor(char.SkillPointsCount());
    if (char.rest_skillPoints == 1) {
      //!!
      char.rest_skillPoints = 0;
    ...
    CharacterlevelSelector.insertAdjacentHTML(
      "beforeend",
      `<option value="${level_reached + 1}">${level_reached + 1}</option>`
    );
    CharacterlevelSelector.selectedIndex += 1;
  }
}
...
function LevelDown(char) {
  console.log("Level lowered to:" + char.level);
  if (char.level > 2) {
    // console.log('tolevel')
    toLevel();
  ...
  function toLevel() {
    if (char.PerksbyLevel.has(char.level)) {
      //Not every level grants new perk
      let perks = char.PerksbyLevel.get(char.level);
...
function LevelJump(char) {
  let chosenLevel = CharacterlevelSelector.value;
  CharacterlevelSelector.selectedIndex = char.level - 1;
  if (chosenLevel > char.level) {
    while (chosenLevel > char.level) {
      LevelUp(char);
    }
  } else if (chosenLevel < char.level) {
    //if
    while (chosenLevel < char.level) {
      LevelDown(char);
    }
...
function SpecialUpDownCheck(char) {
  for (key in char.SPECIAL) {
    if (char.level > 1) {
      ...
      if (
        char.SPECIAL[key].value == char.maxSpecialValue ||
        char.Special_BonusPoints == 0
      ) {
        char.SpecialBlocks.ups.get(key).disabled = true;
      } else {
...
function SkillsUpDownCheck(char) {
  ...
  for (let item of char.skillBlocks.values()) {
    if (char.level == 1) {
      ...
      item.up.disabled =
        skill_points["spent"] == skill_points["points"] ? true : false;

      item.down.disabled =
        !char.skillsByLevel[char.level - 1][item.name] > 0 ? true : false;
    }
  }
  for (let item in char.skills) {
    if (char.skills[item].value(char) == char.maxSkillValue) {
      char.skillBlocks.get(item).up.disabled = true;
    }
  }
}
function update_ChoosenPrizeSkills(char) {
  let skills = char.skillBlocks.keys();
  for (let skill of skills) {
    if (char.ChosenPrizeSkills.has(skill)) {
      char.skillBlocks
        .get(skill)
        .block.classList.replace("unChecked", "Checked");
    } else {
      char.skillBlocks
        .get(skill)
        .block.classList.replace("Checked", "unChecked");
    }
  }
}
function update_ChosenTraits(char) {
  for (let trait of Object.keys(char.traits)) {
    if (char.chosenTraits.has(trait)) {
      char.TraitBlocks.get(trait).classList.replace("unChecked", "Checked");
    } else {
      char.TraitBlocks.get(trait).classList.replace("Checked", "unChecked");
    }
  }
}
function AbilityInfoFill() {
  ...
  let requirements_text =
    perk?.requirements_text !== (undefined || null)
      ? perk.requirements_text
      : "";
