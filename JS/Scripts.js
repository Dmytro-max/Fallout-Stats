    ...
    function update_ChoosenPrizeSkills (char){
        ...
        for (key of keys) {
            if (char.ChosenPrizeSkills.has(key)) {
    ...
    function update_ChosenTraits(char){
        for (let key of Object.keys(char.traits)) {
            if (char.chosenTraits.has(key)) {
                char.TraitBlocks.get(key).classList.replace("unChecked", "Checked");
            }
            else {
                char.TraitBlocks.get(key).classList.replace("Checked", "unChecked");
            }
        }
    }
